...
import { getProSearch } from "@/services/profesionals"
...
export function SearchResults({ searchParams }: SearchResultsProps) {
  const [professionals, setProfessionals] = useState<IProfessionalSearch[]>([])
  ...
  const filteredProfessionals = professionals.filter((prof) => {
    const matchesQuery = searchParams.query
      ? prof.name.toLowerCase().includes(searchParams.query.toLowerCase()) ||
        prof.description.toLowerCase().includes(searchParams.query.toLowerCase())
      : true

    const matchesCategory =
  searchParams.category !== "all"
    ? prof.category?.id === searchParams.category
    : true

    const matchesLocation =
      searchParams.location && searchParams.location !== "all"
        ? prof.city?.toLowerCase() === searchParams.location.toLowerCase()
        : true
                  ...
                  <div className="flex-1 w-full md:flex md:justify-between md:gap-6">
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
                        <div className="flex items-center gap-1">
                          <MapPin className="w-4 h-4" />
                          <span>{getLocationName(professional.city)}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <Award className="w-4 h-4" />
                      ...
                      <div className="flex flex-wrap gap-2">
                        {professional.subcategories.slice(0, 4).map((sub, index) => (
                          <Badge key={index} variant="outline" className="text-xs">
                            {sub.name}
                          </Badge>
                        ))}
                        {professional.subcategories.length > 4 && (
                          <Badge variant="outline" className="text-xs">
                            +{professional.subcategories.length - 4} más
                          </Badge>
                        )}
                      </div>
                    </div>
